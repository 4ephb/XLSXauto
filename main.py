...
@app.route('/test_edit', methods=['GET', 'POST'])
def test_edit():
    global the_table
    if request.method == 'POST':
        new_data = request.form.get("test_table_data")
        the_table = the_table + int(new_data)
        return render_template('test_edit.html', data=the_table)
    return render_template('test_edit.html', data=the_table)
